 ...
 */
function author_redirect_on_activation() {
	global $pagenow;
...
add_action( 'admin_init', 'author_redirect_on_activation' );
 ...
 * since 1.0.0
 */
function author_start_load_admin_scripts() {
...
add_action( 'admin_enqueue_scripts', 'author_start_load_admin_scripts' );
...
class Array_Theme_Updater_Admin {
	...
	function license_page() {
		...
		if( $changelog && !is_wp_error( $changelog ) && 200 === wp_remote_retrieve_response_code( $changelog ) ) {
			...
			$changelog = esc_html__( 'There seems to be a temporary problem retrieving the latest updates for this theme. You can always view the latest updates in your Array Dashboard.', 'author' );
		}
	...
	?>
			...
			<div class="wrap getting-started">
				...
				<div class="intro-wrap">
					<div class="intro">
						<h3><?php printf( esc_html__( 'Getting started with %1$s', 'author' ), $theme['Name'] ); ?></h3>

						<h4><?php printf( esc_html__( 'You will find everything you need to get started with Author below.', 'author' ), $theme['Name'] ); ?></h4>
					</div>
				</div>

				<div class="panels">
					<ul class="inline-list">
						<li class="current"><a id="help-tab" href="#"><?php esc_html_e( 'Help File', 'author' ); ?></a></li>
						<li><a id="updates-tab" href="#"><?php esc_html_e( 'Latest Updates', 'author' ); ?></a></li>
						<li><a id="themes-tab" href="#"><?php esc_html_e( 'More Themes & Discounts', 'author' ); ?></a></li>
					</ul>
					...
					<div id="panel" class="panel">
						...
						<div id="help-panel" class="panel-left visible">
							...
							<?php
								...
								$rss = fetch_feed( 'https://arraythemes.com/articles/author/feed/?withoutcomments=1' );
							...
							<?php if ( is_wp_error( $rss ) || empty( $rss_items_check ) ) : ?>
								<p><?php esc_html_e( 'This help file feed seems to be temporarily down. You can always view the help file on Array in the meantime.', 'author' ); ?> <a href="https://arraythemes.com/articles/<?php echo $theme_name_lower; ?>" title="View help file"><?php echo $theme['Name']; ?> <?php esc_html_e( 'Help File &rarr;', 'author' ); ?></a></p>
							<?php else : ?>
						...
						<div id="themes-panel" class="panel-left">
							<div class="theme-intro">
								<div class="theme-intro-left">
									<p><?php _e( 'Join the Theme Club to download all the themes you see below and new releases for one year for <strong>only <strike>$89</strike> <span>$71.20</span></strong> with the code <strong>THEMEPACK20</strong>!', 'author' ); ?></p>
								</div>
								<div class="theme-intro-right">
									<a class="button-primary club-button" href="<?php echo esc_url('https://arraythemes.com/wordpress-themes/?theme_pack'); ?>"><?php esc_html_e( 'Shop Themes Now', 'author' ); ?> &rarr;</a>
								</div>
							</div>
							...
							if ( ! is_wp_error( $themes_list ) && 200 === wp_remote_retrieve_response_code( $themes_list ) ) {
								...
								printf( __( 'This theme feed seems to be temporarily down. Please check back later, or visit our <a href="%s">Themes page on Array</a>.', 'author' ), esc_url( $themes_link ) );
							} ?>
						...
						<div class="panel-right">
							<div class="panel-aside panel-club">
								<a href="<?php echo esc_url('https://arraythemes.com/wordpress-themes/?theme_pack'); ?>"><img src="<?php echo get_template_directory_uri() . '/includes/admin/getting-started/club.jpg'; ?>" alt="<?php esc_html_e( 'Join the Theme Club!', 'author' ); ?>" /></a>

								<div class="panel-club-inside">
									<h4><?php esc_html_e( 'Instantly download 20+ pixel-perfect WordPress themes!', 'author' ); ?></h4>

									<p><?php esc_html_e( 'Join the Theme Club and download our entire collection of responsive themes, new theme releases and get speedy, expert support &mdash; a massive value!', 'author' ); ?></p>

									<div class="club-discount">
										<p><strong><?php esc_html_e( 'Exclusive 20% Discount!', 'author' ); ?></strong></p>

										<p><?php
											$themes_link = '<code><strong>THEMEPACK20</strong></code>';
											printf( __( 'Use the code %s to get 20&#37; off your next WordPress theme or Theme Club membership!', 'author' ), $themes_link );
										?></p>
									</div>
							...
							<div class="panel-aside">
								...
								<h4><?php esc_html_e( 'Sweet, your license is active!', 'author' ); ?></h4>

								<p><?php esc_html_e( 'You will receive a notice on your Themes page when a theme update is available.', 'author' ); ?></p>

								<?php } else { ?>
									<h4><?php esc_html_e( 'Activate your license to enable theme updates!', 'author' ); ?></h4>

								<p>
									<?php esc_html_e( 'With an active license, you can get seamless, one-click theme updates to keep your site healthy and happy. ', 'author' );

										$license_link = 'https://arraythemes.com/dashboard';
										printf( __( 'Find your license key in your <a target="_blank" href="%s">Array Dashboard</a>.', 'author' ), esc_url( $license_link ) );
									?>
								</p>
								<?php } ?>
								...
								<form class="enter-license" method="post" action="options.php">
									...
									<?php
										...
										<?php } else { ?>
											<small style="font-size:12px;"><?php esc_html_e( 'Be sure to activate your license after saving it.', 'author' ); ?></small><br/><br/>
											<?php if ( $license ) { ?>
	...
	function get_renewal_link() {
		...
		if ( '' != $this->download_id && $license_key ) {
			...
			$url .= '/author/?edd_license_key=' . $license_key . '&download_id=' . $this->download_id;
			return $url;
		}
